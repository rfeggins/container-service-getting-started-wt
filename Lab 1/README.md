...
9. If you created your cluster at the beginning of this, make sure it's ready for use. 
   1. Run `ibmcloud cs clusters` and make sure that your cluster is in "Normal" state.  
   2. Use `ibmcloud cs workers <yourclustername>`, and make sure that all workers are in "Normal" state with "Ready" status.
...
1. Run `ibmcloud cs cluster-config <yourclustername>`, and set the variables based on the output of the command.
